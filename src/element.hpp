...
class ElementTriangular : public Element<P, I>{
	...
	private:
		...
		*/
		...
		precision_t r_u_num(std::vector<precision_t>& coeff_v, 
		...
		precision_t r_u_den(std::vector<precision_t>& coeff_v, 
				precision_t epsilon, precision_t eta){
			precision_t p_1 = phi_1(epsilon, eta);
			precision_t p_2 = phi_2(epsilon, eta);
			precision_t p_3 = phi_3(epsilon, eta);
			precision_t res = 0;
			res += K_MU*(1 + (coeff_v[3]/K_MV + coeff_v[0])*p_1 +
				(coeff_v[4]/K_MV + coeff_v[1])*p_2 +
				(coeff_v[5]/K_MV + coeff_v[2])*p_3);
			res += (coeff_v[0]*coeff_v[3]*pow(p_1, 2) + 
				coeff_v[1]*coeff_v[4]*pow(p_2, 2) +
				coeff_v[2]*coeff_v[5]*pow(p_3, 2))/K_MV;
			res += (coeff_v[0]*p_1*(coeff_v[4]*p_2 + coeff_v[5]*p_3) +
				coeff_v[1]*p_2*(coeff_v[3]*p_1 + coeff_v[5]*p_3) +
				coeff_v[2]*p_3*(coeff_v[3]*p_1 + coeff_v[4]*p_2))/K_MV;
			return res;
		}

		precision_t r_u(std::vector<precision_t>& coeff_v, 
				precision_t epsilon, precision_t eta){
			return r_u_num(coeff_v, epsilon, eta)/
				r_u_den(coeff_v, epsilon, eta);
		}

		precision_t r_v(std::vector<precision_t>& coeff_v, 
				precision_t epsilon, precision_t eta){
			precision_t p_1 = phi_1(epsilon, eta);
			precision_t p_2 = phi_2(epsilon, eta);
			precision_t p_3 = phi_3(epsilon, eta);
			precision_t C_u = p_1*coeff_v[0] + 
				p_2*coeff_v[1] + 
				p_3*coeff_v[2]; 
			return RESP_Q*r_u(coeff_v, epsilon, eta) + 
